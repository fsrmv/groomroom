<?php 
	...
	switch ($view) {
		...
		$orders = set("SELECT * FROM orders INNER JOIN cats ON cats.c_id = orders.o_cat WHERE o_status = 'Услуга оказана' ORDER BY o_timestamp DESC LIMIT 4");
		$count = set("SELECT COUNT(o_id) AS 'count' FROM orders WHERE o_status = 'Услуга оказана'"); // Запросы на завершенные заявки и счетчик
