...
function checkOrders() {
	$orders = set("SELECT COUNT(o_id) AS 'count' FROM orders WHERE o_status = 'Услуга оказана'");
...
function addOrder() {
	if ($_FILES['photo']['size'] > 2097152) {
		...
		$uploadDir = 'E:/programm/openserver/openserver/domains/progect/views/img/';
...
function changeStatus() {
	...
	if (empty($_FILES['photo2'])) {
		...
		global $conn;
		$conn->query("UPDATE orders SET o_status = 'Обработка данных', o_comment = '{$comment}' WHERE o_id = '{$id}'");
		if ($conn->affected_rows > 0) {
			header("Location: /master");
		} else {
			echo $conn->error;
		...
		if (move_uploaded_file($_FILES['photo2']['tmp_name'], $uploadFile)) {
			global $conn;
			$conn->query("UPDATE orders SET o_status = 'Услуга оказана', o_img2 = '{$_FILES['photo2']['name']}' WHERE o_id = '{$id}'");
			if ($conn->affected_rows>0) {
				header("Location: /master");
			} else {
				echo $conn->error;
